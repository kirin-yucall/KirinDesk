...
- **IPv6 / IPv4 直连** — 利用 IPv6 全球可达性，无需 STUN/TURN/中继；IPv6 优先 + IPv4 双栈支持
- **零信任加密** — Ed25519 身份密钥 + X25519 ECDH + AEAD（AES-256-GCM / ChaCha20-Poly1305），每次会话独立派生密钥；握手 pin **强制比对**（已删除"空串跳过"兼容路径）；敏感配置（API 密钥、Token）加密存储（R-13 接线中，当前以明文 + 文件权限告警存储，详见下文"配置加密"）
...
- **内网穿透（Tunnel）** — FRP 式通用 TCP 反向代理：把内网 TCP 服务（SSH/RDP/HTTP…）映射到公网 relay 服务器；SCRAM 式挑战-响应口令认证（口令永不明文上线）、多地址监听、GUI 一键启停并自动恢复；独立 `relay-server` 支持 Docker/systemd/无头部署；可选服务器辅助打洞 + 设备 ID 中继兜底
...
- **安全加固** — 握手 pin 强制比对、配置加密（R-13 接线中）、设备 ID 白名单、SCRAM 式穿透认证、速率限制与审计日志
...
[dns]
provider = "godaddy" # 20 家服务商任选；凭据加密落盘（R-13 接线中）
...
### 配置加密（R-13，M15-T005）——⏳ 接线中

> **当前状态（2026-08-03 审计修正）**：`utils/src/secure.rs`（ChaCha20Poly1305 /
> AES-GCM + DPAPI 主密钥分层 + AAD 上下文绑定）**已实现但尚未接线**——敏感字段
> （GoDaddy API Key/Secret、Tunnel token 等）当前仍以**明文**写入配置文件，
> 依赖文件系统权限保护（config 文件权限告警）。下述密文机制为目标设计，随
> R-13 接线后生效；接线完成前，请勿据此假设配置文件内无明文。

目标设计：GoDaddy API Key/Secret、Tunnel token 等敏感字段**不以明文写入配置文件**，以
ChaCha20Poly1305 密文存储（格式 `{v: base64(nonce‖ciphertext)}`，AAD 绑定
字段上下文，防跨字段替换）。接线完成后配置文件内可 grep 验证无明文。
...
迁移行为（接线后生效）：旧明文配置首次加载时**自动加密重写**（`#[serde(default)]`
兼容旧字段）；迁移失败不破坏原文件（备份 `.bak`）。密钥/令牌不写日志、不进
