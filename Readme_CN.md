...
- **P2P 直连优先** — IPv6 让直连不再依赖 NAT 与端口映射：设备只要持有全球单播地址且前缀可路由，就能直接建立点对点加密隧道，数据不经过任何第三方。实测提醒：一台 IPv6 设备常同时持有多个地址（每接口一个、临时隐私地址、链路本地，甚至多个全球前缀），**并非每个地址都全球可达**——多地址设备上 `ping -6` 不通，往往是因为目标是链路本地 / ULA、过期的临时地址，或前缀并未真正被路由；可达性需要全球单播地址 + 可路由前缀 + 网络放行 ICMPv6/UDP。当直连不可达（NAT/防火墙/地址漂移/选错地址）时，**可选的服务器辅助打洞**（rendezvous 只牵线、不进数据面，打洞成功后仍为双端直连）与**设备 ID 中继兜底**保证连通，端到端加密全程不变。
...
- **IPv6 / IPv4 直连优先** — 利用 IPv6 全球单播地址直连，无需端口映射（注意：设备可能持有多个 IPv6 地址，仅可路由的全球单播地址可达，`ping -6` 不通时先核对地址类型/前缀与防火墙）；v6 建连停滞（默认 3s）自动回退 IPv4；直连不可达时走服务器辅助打洞（rendezvous 仅牵线，数据面双端直连）+ 设备 ID 中继兜底；IPv6 优先 + IPv4 双栈支持
