//! M9-DNS010: DigitalOcean 服务商适配
//!
//! 依据 `M9-DNS000_Provider抽象接口规范.md` 与 `M9-DNS010_DigitalOcean适配.md`。
//!
//! - 认证：`Credential::Digitalocean { token }` → `Authorization: Bearer`
//! - 记录名：**FQDN**（根 "" → 域名本身；读取时 `@` 也按根处理）
//! - 写入语义：单条 CRUD（POST 创建 / PATCH 更新 / DELETE）；upsert = 查(name+type)
//!   → 存在同 data 则 PATCH（含 TTL）、否则 POST（不产生重复）
//! - 分页：`links.pages.next` 跟随遍历
//! - TTL：0 = 服务商默认（直传）
...
/// DigitalOcean Provider（Debug 脱敏）。
pub struct DigitaloceanProvider {
    client: client::DigitaloceanClient,
}

impl std::fmt::Debug for DigitaloceanProvider {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DigitaloceanProvider").field("client", &self.client).finish()
    }
}
...
impl DigitaloceanProvider {
    ...
    pub(crate) fn with_base_url(token: String, base_url: &str) -> Self {
        Self {
            client: client::DigitaloceanClient::new(token, base_url),
        }
    }
}
    ...
    async fn upsert_record(&self, domain: &str, rec: &Record) -> Result<(), ProviderError> {
        ...
        match same {
            Some(v) => {
                ...
                self.client.update_record(domain, &id, &client::record_to_body(rec, domain, false)).await?;
            }
            None => {
                self.client.create_record(domain, &client::record_to_body(rec, domain, true)).await?;
            }
        }
        Ok(())
    }
...
pub fn register(registry: &mut ProviderRegistry) {
    registry.register(
        ...
        |cred| -> Box<dyn Provider> {
            match cred {
                Credential::Digitalocean { token } => Box::new(DigitaloceanProvider::new(token.clone())),
...
impl InvalidCredentialProvider {
    fn new() -> Self {
        Self {
            message: "凭据类型不匹配：digitalocean 需要 Credential::Digitalocean{token}，请检查 [dns.providers.digitalocean] 配置"
                .to_string(),
        }
    }
}
...
mod tests {
    ...
    fn provider(server: &MockServer) -> DigitaloceanProvider {
        DigitaloceanProvider::with_base_url("test-token".to_string(), &server.base_url())
    }
    ...
    #[tokio::test]
    async fn fqdn_conversion_and_root_record() {
        ...
        // 集成：根记录 upsert → POST body name 为域名本身；回读相对名为 ""。
        let srv = MockServer::start().await;
        srv.seed_domain("example.com");
        let p = provider(&srv);
    ...
    async fn delete_removes_record_and_missing_returns_not_found() {
        ...
        srv.seed_record(
            "example.com",
            json!({ "id": "r1", "type": "A", "name": "my-pc.example.com", "data": "203.0.113.7", "ttl": 600 }),
        );
        let p = provider(&srv);
    ...
    async fn error_status_mapping() {
        let boom = r#"{"id":"boom","message":"boom"}"#;
    ...
    async fn srv_structured_fields_roundtrip() {
        ...
        // POST body 形状：name 为 FQDN、target 在 data、SRV 字段独立。
        let reqs = srv.requests();
        let post = reqs.iter().find(|r| r.method == "POST").expect("应有 POST");
        let body: Value = serde_json::from_str(&post.body).unwrap();
        assert_eq!(body["type"], "SRV");
        assert_eq!(body["name"], "_remote._tcp.my-pc.example.com");
        assert_eq!(body["data"], "my-pc.example.com.");
    ...
    async fn pagination_follows_next_links() {
        let srv = MockServer::start().await;
        srv.seed_domain("example.com");
        // mock 每页固定 2 条 → 3 条记录应分 2 页拉全。
        for i in 0..3 {
            srv.seed_record(
                "example.com",
                json!({ "id": format!("r{i}"), "type": "A", "name": format!("h{i}.example.com"), "data": format!("10.0.0.{i}"), "ttl": 600 }),
            );
        }
        let found = provider(&srv).query_records("example.com", None, None).await.unwrap();
        assert_eq!(found.len(), 3, "分页遍历完整：多页记录不遗漏");
        let reqs = srv.requests();
        assert!(
            reqs.iter().any(|r| r.path.contains("page=2")),
            "应跟随 next 链接请求第 2 页: {:?}",
