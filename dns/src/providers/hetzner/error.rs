//! Hetzner DNS 错误映射（M9-DNS013 §三「错误码映射表」）
//!
//! 规则：
...
//! - 其他状态 → `Other`（防御）
//!
//! Hetzner 错误体形状：`{"error": {"code": ..., "message": "..."}}`，文案仅进日志。
...
/// 提取 Hetzner 错误体 message 字段（无则回退摘要）。
fn hetzner_message(body: &str) -> String {
...
pub(crate) fn map_error(status: StatusCode, body: &str) -> ProviderError {
    match status.as_u16() {
        401 | 403 => ProviderError::Auth { detail: hetzner_message(body) },
        400 => ProviderError::InvalidParameter { detail: hetzner_message(body) },
        404 => ProviderError::NotFound { what: hetzner_message(body) },
        429 => ProviderError::RateLimited { retry_after: None },
        500..=599 => ProviderError::Server { status: status.as_u16(), body: summarize(body) },
        other => ProviderError::Other(format!("Hetzner DNS 返回未知状态 {other}: {}", summarize(body))),
    }
}
