...
KirinDesk turns that belief into working software by standing on three pillars that are anything but exotic: **IPv6** gives every device an address space where direct, globally routable connectivity is possible — no NAT gymnastics, no port forwarding; **the DNS system** becomes a decentralized, censorship-resistant bulletin board where devices publish where they are and who they are; and **modern public-key cryptography** replaces the entire legacy TLS certificate apparatus with something simpler, stronger, and truly zero-trust. No certificate authorities. Just devices, DNS, and math — with an **optional self-hosted relay server** ready to assist when the direct path is blocked (see below).
...
IPv6 removes the biggest blocker to direct reachability — NAT and manual port forwarding are gone. One honest caveat from real-world testing: an IPv6-enabled device often holds several addresses at once (one per interface, temporary privacy addresses, link-local, sometimes several global prefixes), and *not all of them are globally reachable* — `ping -6` against a device that has IPv6 addresses can still fail when the address you pinged is link-local, a non-routable ULA, a churned temporary address, or a prefix the network doesn't actually route. Reachability always needs a global unicast address, a routed prefix, and a network that permits ICMPv6/UDP. KirinDesk handles this in layers: domain mode publishes the correct AAAA record, IP mode tries the address you give it, a v6 connect that stalls (default 3 s) automatically falls back to IPv4 — and where no direct path exists, the optional relay server lends a hand as described above: direct first, assisted when necessary, end-to-end encrypted throughout.
