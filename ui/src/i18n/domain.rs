...
pub static TABLE: &[(&str, &str, &str)] = &[
    ...
    ("ddns.interval", "更新周期", "Update interval"),
    ("ddns.interval_unit", "秒", "s"),
    ("ddns.ttl", "TTL", "TTL"),
    ("ddns.ipv4", "IPv4（A 记录）", "IPv4 (A record)"),
    ("ddns.ipv6", "IPv6（AAAA 记录）", "IPv6 (AAAA record)"),
    ("ddns.mode.auto", "自动", "Auto"),
    ("ddns.mode.manual", "手动", "Manual"),
    ("ddns.auto.ipv4", "公网出口 IP：{0}", "Public egress IP: {0}"),
    ("ddns.auto.ipv6", "本机全局单播：{0}", "Global unicast: {0}"),
    ("ddns.auto.none", "（尚未获取到地址）", "(no address yet)"),
    ("ddns.redetect", "重新检测", "Re-detect"),
    ("ddns.detected_at", "获取于 {0}", "fetched at {0}"),
    ("ddns.manual.hint_v4", "输入固定 IPv4（固定出口/映射时填写，心跳不改动）", "Fixed IPv4 for static egress/mapping (never overwritten by heartbeat)"),
    ("ddns.manual.hint_v6", "输入固定 IPv6（上游固定前缀/转发场景）", "Fixed IPv6 for upstream prefix/forwarding scenarios"),
    ("ddns.publish.title", "自动公布", "Auto-publish"),
    ("ddns.publish.srv", "SRV 远控端口 {0}", "SRV remote port {0}"),
    ("ddns.publish.txt", "TXT 签名 {0}", "TXT signature {0}"),
    ("ddns.publish.a", "A", "A"),
    ("ddns.publish.aaaa", "AAAA", "AAAA"),
    ("ddns.publish.off", "未发布", "not published"),
    ("ddns.status.last_ok", "上次更新 {0} 成功（{1}/{2}）", "Last update {0} OK ({1}/{2})"),
    ("ddns.status.last_fail", "上次更新失败: {0}", "Last update failed: {0}"),
    ("ddns.status.never", "尚未执行更新", "No update yet"),
    ("ddns.status.next", "下次更新倒计时 {0}", "Next update in {0}"),
    ("ddns.status.paused", "连续失败已暂停自动更新（手动「立即更新」可解除）", "Auto-update paused after repeated failures (use \"Update now\" to resume)"),
    ("ddns.status.off", "DDNS 已关闭", "DDNS is off"),
    ("ddns.status.disabled", "DNS 服务商未配置 — 请先在上方「服务商」卡填写凭据并保存", "DNS provider not configured — fill in credentials in the Provider card above and save"),
    ("ddns.update_now", "立即更新", "Update now"),
    ("ddns.save", "保存", "Save"),
    ("ddns.saved", "已保存（DDNS 设置即时生效）", "Saved (DDNS settings take effect immediately)"),
    ("ddns.updated", "立即更新完成", "Update now completed"),
    ("ddns.update_failed", "立即更新失败: {0}", "Update now failed: {0}"),
    ("ddns.update_busy", "立即更新中…", "Updating now…"),
    ("ddns.interval_invalid", "更新周期需为 ≥60 的整数秒", "Update interval must be an integer ≥ 60 seconds"),
    ("ddns.manual_invalid_v4", "手动 IPv4 地址格式无效", "Invalid manual IPv4 address"),
    ("ddns.manual_invalid_v6", "手动 IPv6 地址格式无效", "Invalid manual IPv6 address"),
];
